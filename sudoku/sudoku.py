class Sudoku:
    ...
    def check_elements(self, info_position):
        info = {"fila": info_position["fila"], "cuadricula": info_position["cuadricula"],
                "fila_cuadricula": self.get_fila_cuadricula(info_position["posicion"]),
                "posicion_fila_cuadricula": self.get_position_fila_cuadricula(info_position["posicion"]),
                "valor": info_position["valor"]}
    ...
    def insert_element(self, info_position) -> object:
        info = {"fila": info_position["fila"], "cuadricula": info_position["cuadricula"],
                "fila_cuadricula": self.get_fila_cuadricula(info_position["posicion"]),
                "posicion_fila_cuadricula": self.get_position_fila_cuadricula(info_position["posicion"]),
                "valor": info_position["valor"], "tablero": info_position["tablero"]}
        is_correct = False
        try:
            for row in info["tablero"]:
                if row["fila"] == info["fila"]:
                    row["columnas"][info["cuadricula"]][info["fila_cuadricula"]][info["posicion_fila_cuadricula"]] = info["valor"]

            info_position["tablero"] = info["tablero"]
